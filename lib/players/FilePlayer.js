...
var HLS_SDK_URLS = ['https://cdn.jsdelivr.net/npm/hls.js@latest/dist/hls.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/hls.js/0.12.4/hls.min.js'];
var HLS_GLOBAL = 'Hls';
  ...
  _createClass(FilePlayer, [{
      ...
      if (this.shouldUseHLS(url)) {
        ...
        (0, _utils.getSDK)(hlsUrls[retries], HLS_GLOBAL).then(function (Hls) {
          ...
          if (err) {
            ...
            if (retries < 0) {
              throw new Error("Hls is not loading from ".concat(_this2.getLibraryUrlArray(HLS).join(' ')));
            } else {
              setTimeout(function () {
      ...
      if (this.shouldUseFLV(url)) {
        (0, _utils.getSDK)(FLV_SDK_URL.replace('VERSION', flvVersion), FLV_GLOBAL).then(function (flvjs) {
          ...
          if (err) {
            ...
            if (retries < 0) {
              throw new Error("Dash is not loading from ".concat(_this2.getLibraryUrlArray(DASH).join(' ')));
            } else {
              setTimeout(function () {
                _this2.load(url, retries);
              }, 1000);
    ...
    value: function play() {
      try {
        ...
        throw new Error("FilePlayer error trying to play video: ".concat(err.message));
      }
    }
  }, {
    key: "pause",
